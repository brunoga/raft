...
// client reaches the in-process listeners startLedger put the nodes on.
// "n1:8080" is not an address the operating system can route, so the ordinary
// http.DefaultClient cannot be used here.
...
func startLedger(t *testing.T) (cluster *easyrafttest.Cluster, addrs []string) {
	...
	// durably blocked, and one idle listener is enough to stop a bubble's
	// clock for good.
...
func setUp(t *testing.T, base string) {
	...
	for !opened && time.Now().Before(deadline) {
		status, body, _ := do(t, http.MethodPost, base+"/period", `{"id":"2026-09"}`)
		lastStatus, lastBody = status, body
		opened = status == http.StatusNoContent
		if !opened {
			time.Sleep(20 * time.Millisecond)
		}
	}
	// Saying so here rather than letting the next request fail: this loop used
	// to fall out silently, and the period never opening then surfaced as a
	// 503 from POST /accounts, which is not where the problem was.
	if !opened {
		t.Fatalf("the period never opened: last answer %d %s", lastStatus, lastBody)
	}
