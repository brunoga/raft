...
func TestRestart_FollowerCatchesUpAfterRestart(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		// The follower is stopped and replaced partway through, so this stops
		// whatever is in the slice at the end rather than the nodes built
		// here. A node still running when the bubble ends is a failure.
		defer func() {
			for _, n := range nodes {
				if n != nil {
					n.Stop()
